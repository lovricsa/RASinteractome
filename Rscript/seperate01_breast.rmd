...
TO DO: FFP-s mintát már az elején kiszedni, Stage X-eket már az elején kiszedni
...
ras_data <- fread(file.path(datafolder, 
                            ...
                            "TCGA_BRCA_plus_stage_20241028.tsv"),
                     sep="\t")
...
ras_data$category2[ras_data$category2 %in% c("Normal_Stage_I",
...
saveRDS(list("data"=ras_data, "indep_vars"=indep_vars),
        file=file.path(calcfolder, paste("TCGA_", rtype, ".rds")))
...
# Compare outliers with incorrectly classified samples by LDA
```{r LDA misclassified vs outliers}
mi <- which(!lda_values$class == ras_data$sample_type_factor)
msamples <- lda_annot$sample[mi]
osamples <- pca_annot$sample[which(pca_annot$outlier=="YES")]
intersect(msamples, osamples)
```
...
# best model finding with findAIC
```{r use stepAIC}
...
<!-- # for the model, add best gene from each cluster -->
<!-- ```{r repeated model building} -->
<!-- model_names <- list() -->
<!-- for (ii in 1:length(neworder_names)){ -->
<!--   tmp_aic <- single_aic[which(names(single_aic) %in% neworder_names[[ii]])] -->
<!--   ai <- which(tmp_aic==min(tmp_aic)) -->
<!--   sel_var <- names(tmp_aic)[ai] -->
<!--   model_names[[ii]] <- sel_var -->
<!-- } -->
<!-- model_names <- unlist(model_names) -->
<!-- suppressWarnings(test_model <-  glm(as.formula(paste("sample_type_bool ~ ", -->
<!--                     paste0(model_names, collapse="+"), sep="")), -->
<!--                         data=ras_data, family="binomial")) -->
<!-- print("AIC:") -->
<!-- print(test_model$aic) -->
<!-- print("VIF:") -->
<!-- print(car::vif(test_model)) -->

<!-- # now test the best model -->
<!-- set.seed(7001) -->
<!-- numCV_outer <- 5 -->
<!-- numCV_inner <- 5 -->
<!-- hold_rows_list <- createFolds(ras_data$sample_type_factor, -->
<!--   k=numCV_outer, list = T) -->
<!-- logreg_model_best_list <- list() -->
<!-- logreg_model_best_resp <- factor(rep("Tumor", times=nrow(ras_data)),  -->
<!--                                             levels=c("Normal", "Tumor")) -->
...
<!--       inner_train_list <- createFolds(ras_train$sample_type_factor,  -->
...
<!--     as.formula(paste("sample_type_factor ~ ", -->
