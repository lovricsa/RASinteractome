...
# ## ----remove outliers using the ROUT method (~PRISM)--------------------------
# for (rtype in c("COAD", "LUAD", "LUSC")){
#   ras_data <- ras_data_list[[rtype]]
#   
...
#   ggsave(resfile, plot=pfinal, width=20, height=12)
#   
#   # use pca_annot instead of ras_data
#   outlier_df <- matrix(data=0, nrow=nrow(ras_data), ncol=length(indep_vars),
#                         dimnames=list(ras_data$sample, indep_vars))
#   # repeat outlier search for each stage
#   for (category in unique(ras_data$category_factor)){
#     if (category=="Tumor_Stage_X") next
#     ras_tmp <- ras_data[ras_data$category_factor==category,]
#     ras_tmp$category_factor <- droplevels(ras_tmp$category_factor)
#     M_indep_vars <- apply(ras_tmp[, indep_vars], 2, median)
#     # ROUT
#     out_sample_all <- c()
#     for (ii in 1:length(indep_vars)){
#       resids <- ras_tmp[, indep_vars][,ii, drop=T] - M_indep_vars[ii]
#       res_ind <- OutlierDetection(resids)
#       out_sample <- ras_tmp$sample[res_ind]
#       outlier_df[which(rownames(outlier_df) %in% out_sample), ii] <- 1
#       # tt <- tryCatch(OutlierDetection(resids),error=function(e) e, warning=function(w) w)
#       # if(is(tt,"warning")){
#       #   print("warning outlier detection")
#       #   print(category)
#       #   print(ii)
#       # }
#       # if (length(tt)>0){
...
#       #   print(indep_vars[ii])
#       # }
#       out_sample_all <- c(out_sample_all, out_sample)
#     }
#     print(category)
#     print(unique(out_sample_all))
#     ri <- which(rownames(outlier_df) %in% out_sample_all)
#     print(outlier_df[ri, ])
#     print(rowSums(outlier_df[ri, ,drop=F]))
#     print("---------------#####--------------------")
#     
#   } # for each category
#   
#   # use pca_annot instead of ras_data
#   pca_vars <- colnames(pca_annot)[2:
#         (which(colnames(pca_annot)=="pathologic_stage")-1)]
#   outlier_df <- matrix(data=0, nrow=nrow(ras_data), ncol=length(pca_vars),
#                        dimnames=list(pca_annot$sample, pca_vars))
#   # repeat outlier search for each stage
#   for (category in unique(pca_annot$category_factor)){
#     if (category=="Tumor_Stage_X") next
#     ras_tmp <- pca_annot[pca_annot$category_factor==category,]
#     ras_tmp$category_factor <- droplevels(ras_tmp$category_factor)
#     M_pca_vars <- apply(ras_tmp[, pca_vars], 2, median)
#     # ROUT
#     out_sample_all <- c()
#     for (ii in 1:length(pca_vars)){
#       resids <- ras_tmp[, pca_vars][,ii, drop=T] - M_pca_vars[ii]
#       res_ind <- OutlierDetection(resids)
#       out_sample <- ras_tmp$sample[res_ind]
#       outlier_df[which(rownames(outlier_df) %in% out_sample), ii] <- 1
#       out_sample_all <- c(out_sample_all, out_sample)
...
# }
# 
# # COAD TCGA-AZ-6605-11 (Normal) - outlier in the case of 10 genes
# 
...
for (rtype in c("COAD", "LUAD", "LUSC")){
  ...
  for (stype in c("Normal", "Tumor")){
    ...
    # TO DO: save plots with & without legend
    # 600 dpi TIFF
...
plot_corrs <- cowplot::plot_grid(plot_list[["COAD"]][["Normal"]],
                                 plot_list[["LUAD"]][["Normal"]],
                                 plot_list[["LUSC"]][["Normal"]],
                                 plot_list[["COAD"]][["Tumor"]],
                                 plot_list[["LUAD"]][["Tumor"]],
                                 plot_list[["LUSC"]][["Tumor"]],
                   ncol=3)
                   #labels = c("A", "B", "C", "D", "D", "F"))
outfig <- file.path(figfolder, "correlation_together_normal_vs_tumor.pdf")
ggsave(outfig, plot=plot_corrs, height=20, width=28)
...
for (rtype in names(corr_list)){
  ...
  mean_connections[[rtype]][["Tumor"]] <- mean(
    abs(corr_list[[rtype]][["Tumor"]]))
  mean_connections_diff[[rtype]] <- round(mean_connections[[rtype]][["Normal"]] -
                                            mean_connections[[rtype]][["Tumor"]], digits=2)
...
for (rtype in c("COAD", "LUAD", "LUSC")){
  num_list[[rtype]] <- list()
  num_list[[rtype]][["Normal"]] <- c(rev(num_connections_neg_norm[[rtype]]),
                                     num_connections_pos_norm[[rtype]])
  num_list[[rtype]][["Tumor"]] <- c(rev(num_connections_neg_tum[[rtype]]),
  ...
  num_list[[rtype]]$diff <-  num_list[[rtype]]$Normal -  num_list[[rtype]]$Tumor
}

# TO DO - abs pearson: 0.4-(0.05 lépés)-1-ig  - két külön ábra egymás mellett
# próba: 0.2-től vagy 0.4-től
# color: black & red, linewidth=2, also geom_point
# background: white
# y-tengely: number of connections
# x-tengely: Pearson correlation threshold
# mellé: lost connections (negatív külön)
# mean(abs(cor)) táblázatban
# plus ezek az adatok is táblázatban
p <- ggplot(data=num_list[["COAD"]])+
   geom_line(mapping=aes(x=corr, y=Normal), color="black", linewidth=2)+
  geom_line(mapping=aes(x=corr, y=Tumor), color="red", linewidth=2)+
  geom_line(data=num_list[["LUAD"]], mapping=aes(x=corr, y=Normal), color="black")+
  geom_line(data=num_list[["LUAD"]], mapping=aes(x=corr, y=Tumor), color="red")+
  geom_line(data=num_list[["LUSC"]], mapping=aes(x=corr, y=Normal), color="black", linetype="dashed")+
  geom_line(data=num_list[["LUSC"]], mapping=aes(x=corr, y=Tumor), color="red", linetype="dashed")+
  geom_vline(xintercept=c(-0.2, 0.2))
plot(p)

p <- ggplot(data=num_list[["COAD"]])+
  geom_line(mapping=aes(x=corr, y=diff), color="black", linewidth=2)+
  geom_line(data=num_list[["LUAD"]], mapping=aes(x=corr, y=diff), color="black")+
  geom_line(data=num_list[["LUSC"]], mapping=aes(x=corr, y=diff), color="black", linetype="dashed")+
  geom_vline(xintercept=c(-0.2, 0.2))
plot(p)
...
for (rtype in names(corr_list)){
  plist[[pindex]] <- ggplot(data=num_list[[rtype]])+
    geom_line(mapping=aes(x=corr, y=Normal), color="black",
    linewidth=linewidth_vector[pindex], linetype=linetype_vector[pindex])+
    geom_point(mapping=aes(x=corr, y=Normal), color="black", size=psize)+
    geom_line(mapping=aes(x=corr, y=Tumor), color="red",
    linewidth=linewidth_vector[pindex], linetype=linetype_vector[pindex])+
    geom_point(mapping=aes(x=corr, y=Tumor), color="red", size=psize)+
    #xlim(-0.8,-rlim)+ 
    ylim(0,ylim1) +
    ...
    theme(axis.title.x=element_blank(),
                   axis.text.x=element_blank(),
                   axis.ticks.x=element_blank()) %+replace%
    la_replace +
    labs(title = rtype)
  #plot(plist[[pindex]])
...
for (rtype in names(corr_list)){
  plist_bottom[[pindex]] <- ggplot(data=num_list[[rtype]])+
    ...
    ylab("Lost connections")+ xlab("correlation threshold")+
    # theme(axis.text=element_text(size=16),
    #       axis.title=element_text(size=20))
  theme() %+replace%
    la_replace
...
plot(p_all)
outfig <- file.path(figfolder, "lost_connections_minus_plus_seperately.pdf")
ggsave(outfig, plot=p_all, width=21, height=14)

# write out to file
outfile <- file.path(resfolder, "lost_connections_minus_plus_seperately.xlsx")
...
for (rtype in names(corr_list)){
  plist[[pindex]] <- ggplot(data=num_list[[rtype]])+
    ...
    geom_line(mapping=aes(x=corr, y=diff), color="darkgreen",
              linewidth=linewidth_vector[pindex], linetype=linetype_vector[pindex])+
    geom_point(mapping=aes(x=corr, y=diff), color="darkgreen", size=psize)+
    xlim(-0.8,-rlim)+ ylim(0,ylim1) +
    ylab("Number of connections")+ xlab("correlation threshold")+
    theme(axis.text=element_text(size=16),
          axis.title=element_text(size=20))+
    # plot.title = element_text(hjust = 0.8, vjust = -10, size = 16),
    # plot.title.position = "plot")+
...
p_top <- cowplot::plot_grid(plotlist=plist, nrow=1)
plot(p_top)
outfig <- file.path(figfolder, "lost_connections_minus_plus_seperately_withdiff.pdf")
ggsave(outfig, plot=p_top, width=21, height=10)
...
#plot(p)
figfile <- file.path(figfolder, "correlation_lost_connections_normal_vs_tumor.pdf")
...
plot_corrs <- cowplot::plot_grid(plot_list[["COAD"]][["Normal"]],
                                 plot_list[["LUAD"]][["Normal"]],
                                 plot_list[["LUSC"]][["Normal"]],
                                 plot_list[["COAD"]][["Tumor_Stage_I"]],
                                 plot_list[["LUAD"]][["Tumor_Stage_I"]],
                                 plot_list[["LUSC"]][["Tumor_Stage_I"]],
                                 ncol=3)
                                 #labels = c("A", "B", "C", "D", "D", "F"))
outfig <- file.path(figfolder, "correlation_together_normal_vs_stageI.pdf")
ggsave(outfig, plot=plot_corrs, height=20, width=28)
...
## ----LDA stages but use Normal and Stages I and IV only for prediction---------------------------------------
for (rtype in c("COAD", "LUAD", "LUSC")){
  ras_data <- ras_scaled_list[[rtype]]
  ...
  outfig <- file.path(figfolder, rtype, "lda_scaling.pdf")
