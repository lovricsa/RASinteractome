...
######################
for (comptype in c("normal_vs_tumor", "normal_vs_stageI", 
                   "stageI_vs_stageIV")){
  summ_df_list <- list()
  for (rtype in c("COAD")){
  #for (rtype in c("BRCA", "COAD", "LUAD", "LUSC")){
    for (num_genes in c(3, 4)){
      # original data
      # read data
      ...
      figfile <- file.path(figfolder, rtype, 
                           paste(rtype, "best_", as.character(num_genes), 
                          "gene_aic_bic_acc_counts_", comptype,".pdf", sep=""))
      ...
      summ_df <- as.data.frame(reslist[c("aic_vals", "bic_vals", "acc_vals")])
      summ_df$aic_vals <- round(summ_df$aic_vals, digits=0)
      summ_df$bic_vals <- round(summ_df$bic_vals, digits=0)
      summ_df$acc_vals <- round(summ_df$acc_vals, digits=2)
      summ_df <- as_tibble(summ_df)
      summ_df$sel_genes <- as.character(NA)
      for (ii in 1:nrow(summ_df)){
        curr_mod <- reslist[["mod_list"]][ii][[1]]
        sel_genes <- names(curr_mod$coefficients)[2:length(curr_mod$coefficients)]
        summ_df$sel_genes[ii] <- paste0(sel_genes,collapse="+")
      }
      
      # sort by accuracy
      summ_df <- summ_df[order(summ_df$acc_vals, decreasing = T), ]
      lname <- paste(rtype, "_", as.character(num_genes), sep="")
      summ_df_list[[lname]] <- summ_df
    } # loop for num_genes
  ...
  outfile <- file.path(resfolder, 
                       paste("best_", 
                          "gene_aic_bic_acc_counts_", comptype,".xlsx", sep=""))
  openxlsx::write.xlsx(summ_df_list, file=outfile)
}
